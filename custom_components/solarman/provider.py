...
class ConfigurationProvider:
    ...
    @cached_property
    def port(self):
        return self._options.get(CONF_PORT)

    @cached_property
    def transport(self):
        return self._options.get(CONF_TRANSPORT)

    @cached_property
    def filename(self):
        return self._options.get(CONF_LOOKUP_FILE)

    @cached_property
    def mb_slave_id(self):
        return self._options.get(CONF_MB_SLAVE_ID, DEFAULT_[CONF_MB_SLAVE_ID])
...
class ProfileProvider:
    ...
    async def resolve(self, request: Callable[[], Awaitable[None]] | None = None):
        ...
        if (f := await lookup_profile(request, self.parameters) if self.auto and request else self.filename) and (n := process_profile(f, self.parameters)) and (p := await yaml_open(self.config.directory + n)):
            self.parser = ParameterParser(p, self.parameters)
            self.info = (unwrap(p["info"], "model", self.parameters) if "info" in p else {}) | {"filename": f}
